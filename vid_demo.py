...
def get_start_vid(vid_res_root, vid_data_root):
    ...
    for _ in range(len(vid_data_list)):
        if i>= len(vid_res_list) or vid_res_list[i] == vid_data_list[i].split('.')[0]:
            i += 1
            continue
        else:
            break

    print('start from {:03d}th video, [{}]'.format(i-1, vid_res_list[i-1]))
    return max(i-1, 0)
...
def saveVid(demo, vidName, vidSrcRoot, vidResRoot, detResRoot, proc_bar = None):
    vid_masks = []
    vid_boxes = []
    vid_scores = []
    ...
    for i in range(len(video)):
        ...
        vid_result_path = os.path.join(vidResRoot, vidName, '{:03d}.jpg'.format(i+1))
...
if __name__ == "__main__":
    ...
    vid_result_root = './vid_result'
    vid_source_root = './videos-pirates/videos'
    ...
    for vid_idx in proc_bar:
        ...
        saveVid(demo, vid_name, vid_source_root, vid_result_root, det_result_root, proc_bar=proc_bar)
